monogatari.label('enPart3', 'English', [
  'show scene sky with fadeIn',
  'In a country called Japan, a man named YAGOO, who is the CEO of COVER Corporation and the owner of a vtuber agency Hololive, is worried, because his dreams are being crushed by the Talents he have right now.',  
  'show scene office with fadeIn',
  'clearall',
  // 'wait',
  'wait 3000',
  "play music YagooOffice with loop volume 10",
  'show scene inoffice with fadeIn',
  "wait 1000",
  'show character yagoo normal with fadeIn',
  "play voice Yagoo_01",
  'yagoo Looks like the Talents are getting ever further from the word "idol"… They are more of a comedians now.',
  'show character yagoo cry',
  "play voice Yagoo_02",
  'yagoo I don’t think they are able to realize my dream of v-idol that is like AKB-48. People even started calling them "AK-47."',
  'show character yagoo normal',
  "play voice Yagoo_03",
  "yagoo I've heard that in Indonesia there are many talented people.",
  'show character yagoo smile',
  "play voice Yagoo_04",
  'yagoo Maybe there I could find talents who can become perfect idols. who can make my dream come true.',
  'show character yagoo shinySmile',
  'clearall',
  'wait 2000',
  'hide character yagoo shinySmile with fadeOut',
  'Then he tries his luck searching for a talent in a country called Indonesia.',  
  "stop music YagooOffice with fade 5",
  'show scene sky with fadeIn',
  "wait 1000",
  "play sound city with loop volume 8",
  'Iofi is walking back to home from her college - Asacoco University branch Pancoran Pinggir Dalam.',
  "play music YagooSus with loop volume 9",
  "play sound ServantSweeping loop with volume 10",
  'show background sus1 with fadeIn',
  'When she walks, she notices a stand with a man that gives away flyers with printed headline "Hololive Indonesia."',
  'show background sus2',
  'show character iofi bingung with fadeIn',
  'iofi Hololive? Isn’t that a v-tuber agency from Japan? Is Hololive opening a branch in Indonesia?',
  'show character iofi senyum with fadeIn',
  'iofi May I have one of the flyers?',
  'hide character iofi senyum with fadeOut',
  'show character susyagoo shinySmile with fadeIn',
  "play voice Yagoo_05",
  'susyagoo Of course, young lady~ Here, take it.',
  'hide character susyagoo shinySmile with fadeOut',
  'show character iofi senyumCanggung with fadeIn',
  'iofi Thank you~✨',
  'hide character iofi senyumCanggung with fadeOut',
  'Iofi then took the flyer given to her and she brought it home.',  
  "stop sound city",
  "stop sound ServantSweeping",
  "stop music YagooSus with fade 5",
  "play music IofiRecording with loop volume 10",
  'show scene dorm with fadeIn',
  "wait 1000",
  "play music IofiRecording with loop volume 10",
  'show scene dorm with fadeIn',
  // == Membaca Selembaran disini ==,
  'show character iofi normal with fadeIn',
  'iofi So I have to send a short video about the talents I have?',
  'iofi Why has my confidence disappeared, just after reading the flyer?',
  'iofi No, no, no, Iofi. You must think positively.',
  'iofi Okay, Iofi, you can do it! Even if you are only good at drawing and learning new languages, you can do it!',
  'hide character iofi normal with fadeOut',
  'After building up her self-confidence, Iofi have a courage to make a short video about her talents and send it to Hololive ID.',
  'show character iofi bahagia with fadeIn',
  'iofi Hello everyone, my name is Airani Iofifteen.....',
  "stop music IofiRecording with fade 10",
  'show scene #000',
  'centered Several months later',
  'show scene dorm with fadeIn',
  'wait 1000',
  "play sound CallFromMChan loop",
  '<i>Kring........</i>',
  "stop sound CallFromMChan",
  'show character iofi normal with fadeIn',
  'iofi Yes, hello? Who is this?',
  "play voice M-chan_01",
  'unknown Is this the correct number for Airani Iofifteen?',
  'show character iofi bingung',
  'iofi Yes. It’s me.',
  "play voice M-chan_02",
  'unknown First, let me introduce myself. I’m M-chan from Hololive ID.',
  'show character iofi senyum',
  'iofi Hololive Indonesia? Is this really Hololive Indonesia?',
  'show ext-background comical with slideInLeft 1000',
  'show character iofi senyum at right with slideInLeft',
  'show character mchan normal at left with slideInLeft',
  "play music M-chan with loop volume 5",
  "play voice M-chan_03",
  'mchan Yes, it is. I am M-chan from Hololive Indonesia.',
  "play voice M-chan_04",
  'mchan I would like to tell you about the audition result.',
  "stop music M-chan with fade 10",
  "play voice M-chan_05",
  'mchan So, we from Hololive ID, declares that Airani Iofifteen...',
  'show character iofi normal at right',
  "play sound heartbeat loop",
  'iofi <i>Pass…? Failed....?</i>',
  "stop sound heartbeat",
  "play voice M-chan_06",
  'mchan Too bad.......',
  'show character iofi panik at right',
  'iofi ...',
  "play voice M-chan_07",
  'mchan You have passed the Hololive Indonesia audition!',
  'show character iofi bingung at right',
  'iofi Have I really passed?',
  "play voice M-chan_08",
  'mchan Yes... Congratulations..... For further information, we will contact you a few days later.',
  'show character iofi bahagia at right',
  'iofi Thank you....Thank you....',
  'show ext-background comical with slideOutLeft',
  'hide character mchan normal at left with slideOutLeft',
  'show character iofi bahagia with slideInRight',
  "play music M-chan with loop volume 5",
  'iofi I-I passed? Is this a dream? I passed! Iofi, you did it.',
  'hide character iofi bahagia with fadeOut',
  'With all her abilities, and maybe a little bit of “brainwash”, she finally passed and became a talent of Hololive Indonesia.',
  "stop music M-chan with fade 10",
  'show scene #000',
  'wait 1000',
  "play music CreditsSong volume 10",
  {
    'Credit': {
      'fade' : [
        {
          'title': "Project Initiator",
          'content': "Para Haluforia"
        },
        {
          'title': "Cast",
          'list': {
            'King': "Miko 草ガキ",
            'Queen': "Zuvanix",
            'Second Princess': "Api Atsu"
          }
        },
        {
          'title': "",
          'list': {
            'Envoy': "Khateas Raktea",
            'Advisor': "Iofi's Personal Chair",
            'Emperor': "Dr.Nergigante"
          }
        },
        {
          'title': "",
          'list': {
            'M-chan': "ApiAtsu",
            'Agent Kuda': "Minyak Sayur",
            'Agent Kecoa': "Vesera"
          }
        },
        {
          'title': "",
          'list': {
            'MIB 1': "Yuta",
            'MIB 2': "Enterprise",
            'YAGOO': "Hitsuji(cpp)"
          }
        }
      ],
      'scroll': [
        {
          'title': "Cast",
          'list': {
            'King': "Miko 草ガキ",
            'Queen': "Zuvanix",
            'Second Princess': "ApiAtsu",
            'Emperor': "Dr.Nergigante",
            'Advisor': "PunPun Iofi's Personal Chair",
            'Envoy': "Raktea",
            'Agent Kuda': "Minyak Sayur",
            'Agent Kecoa': "Vesera",
            'MIB 1': "Yuta",
            'MIB 2': "Enterprise",
            'Yagoo': "Hitsuji(cpp)",
            'M-chan': "ApiAtsu",
          }
        },
        {
          'title': "Story",
          'list': {
            'Originally Written By': "Rudi TM",
            'Translated By': [
              "Vesera (EN)",
              "Reiko (JP)"
            ],
            'Editor and Quality Check': [
              "Yuta Zoka",
              "Khateas Raktea",
              "Dr.Nergigante",
              "Minyak Sayur",
              "Hitsuji(cpp)",
              "LegoVW"
            ]
          }
        },
        {
          'title': "Background and Illustration",
          'list': [
            "Transenponsan",
            "Yugen",
            "Laeventien",
            "PunPun Iofi's Personal Chair",
            "JeffriArt",
            "Miko 草ガキ",
            "Littlecrownie",
            "Hacto_17"
          ]
        },
        {
          'title': "Character Design",
          'list': [
            "JeffriArt",
            "Chassy Sant",
            "Laeventien",
            "Haloclo 18",
          ]
        },
        {
          'title': "Programming" ,
          'list': [
            "Moonchild",
            "JeffriArt",
            "Zainokta Rakei",
            "KucingLaut",
          ]
        },
        {
          'title': "Tester",
          'list' : [
            "Asukiri",
            "Chero",
            "LegoVW",
            "Pika",   
            "Azureos",
            "Arcana",
            "Chumacera",
            "Feeltis Catcus",
            "Kisaragi Ryuu",
            "Rad",
            "Rediffract",
            "Ryokhan",
          ]
        },
        {
          'title': "Background Music",
          'subtitle': "DOVA",
          'class': "credit-bgm-section",
          'list': [
            {
              "[Machina]": "だんご工房",
              "[珊瑚礁]": "伊藤ケイス",
              "[隠された遺跡の中へ]": "秦暁",
              "[Clouded Stage]": "MFP [Marron Fields Production]",
              "[淵に立つ背中を押すのは...的なBGM]": "鷹尾まさき[タカオマサキ]",
              "[哀愁漂うホラー的なBGM]": "鷹尾まさき[タカオマサキ]",
              "[Digital Space]": "Shimtone",
              "[Painted from Memory ～想いは調べにのって～]": "田中芳典"
            },
            {
              "[閉ざされた海底都市]": "ハヤシユウ",
              "[堕ちる]": "もっぴーさうんど",
              "[Ruins]": "Motomuge",
              "[Tension]": "こおろぎ",
              "[帰ろう]": "T12ya",
              "[日曜の午後]": "KK",
              "[Funny Life]": "T.tam",
              "[South Wind]": "FLASH☆BEAT"
            }
          ]
        },
        {
          'title': "Ending Music Theme",
          'subtitle': "「Bersama Ioforia」",
          'list': {
            'Lyrics': [
              "Airani Iofifteen Channel hololive-ID",
              "Rikka ch.律可"
            ],
            'Music': "Rikka ch.律可",
            'Arrangement': 'Zuvanix'
          }
        },
        {
          'title': "Special Thanks",
          'content': [
            "Zad",
            "Binyo Boss", 
            "GrilledCheez",
            "Melon Juice", 
            "Rarana", 
            "Zuvanix", 
            "Kyarudesu",
            "Mas Ittou",
            "And All Beloved Ioforia..."
          ]
        },
      ],
      'quote': {
        'content': [`"Kehidupan sama dengan lukisan cat minyak. Kesalahanya memang tidak bisa dihapus, tapi bisa diperbaiki."`],
        'source': "Agnes Jessica"
      }
    }
  },
  'centered Several months later',
  'show scene galaxy with fadeIn',
  'show character raja takut with fadeIn',
  "play voice King_20",
  'king They have a massive fleet, can we really defeat them?',
  'show character raja percaya with fadeIn',
  "play voice King_21",
  'king No! I must never show my fear in front of the soldiers.',
  'show character raja teriak2 with fadeIn',
  "play voice King_22",
  'king Soldiers, get ready in your positions! This is going to be a hard battle, but we will show them that Iolyptos is an independent nation.',
  "play voice King_23",
  'king Do not be afraid of the emperor and believe that we can defeat their fleet!',
  'hide character raja teriak with fadeOut',
  'soldier My Lord, there is another fleet entering the battlefield.',
  'show character raja kaget with fadeIn',
  "play voice King_24",
  'king Go check whose flee-',
  'hide character raja normal with fadeOut',
  'show scene ufo with fadeIn',
  "play voice Orang-Asing_01",
  'unknown Hey brat, I will help you this time. At least I am not going to let you die easily. I still want to beat you after taking my daughter before, and now you repatriate her back to me. ',
  "play voice Orang-Asing_02",
  'unknown Prepare yourself, if you want to stay alive after this battle!',
  'show scene #000',
  'wait 1500',
  'centered To Be Continued...?',
  'end'
]);

monogatari.label('credit', 'English', [
  "play music CreditsSong volume 10",
  {
    'Credit': {
      'fade' : [
        {
          'title': "Project Initiator",
          'content': "Para Haluforia"
        },
        {
          'title': "Cast",
          'list': {
            'King': "Miko 草ガキ",
            'Queen': "Zuvanix",
            'Second Princess': "Api Atsu"
          }
        },
        {
          'title': "",
          'list': {
            'Envoy': "Khateas Raktea",
            'Advisor': "Iofi's Personal Chair",
            'Emperor': "Dr.Nergigante"
          }
        },
        {
          'title': "",
          'list': {
            'M-chan': "ApiAtsu",
            'Agent Kuda': "Minyak Sayur",
            'Agent Kecoa': "Vesera"
          }
        },
        {
          'title': "",
          'list': {
            'MIB 1': "Yuta",
            'MIB 2': "Enterprise",
            'YAGOO': "Hitsuji(cpp)"
          }
        }
      ],
      'scroll': [
        {
          'title': "Cast",
          'list': {
            'King': "Miko 草ガキ",
            'Queen': "Zuvanix",
            'Second Princess': "ApiAtsu",
            'Emperor': "Dr.Nergigante",
            'Advisor': "PunPun Iofi's Personal Chair",
            'Envoy': "Raktea",
            'Agent Kuda': "Minyak Sayur",
            'Agent Kecoa': "Vesera",
            'MIB 1': "Yuta",
            'MIB 2': "Enterprise",
            'Yagoo': "Hitsuji(cpp)",
            'M-chan': "ApiAtsu",
          }
        },
        {
          'title': "Story",
          'list': {
            'Originally Written By': "Rudi TM",
            'Translated By': [
              "Vesera (EN)",
              "Reiko (JP)"
            ],
            'Editor and Quality Check': [
              "Yuta Zoka",
              "Khateas Raktea",
              "Dr.Nergigante",
              "Minyak Sayur",
              "Hitsuji(cpp)",
              "LegoVW"
            ]
          }
        },
        {
          'title': "Background and Illustration",
          'list': [
            "Transenponsan",
            "Yugen",
            "Laeventien",
            "PunPun Iofi's Personal Chair",
            "JeffriArt",
            "Miko 草ガキ",
            "Littlecrownie",
            "Hacto_17"
          ]
        },
        {
          'title': "Character Design",
          'list': [
            "JeffriArt",
            "Chassy Sant",
            "Laeventien",
            "Haloclo 18",
          ]
        },
        {
          'title': "Programming" ,
          'list': [
            "Moonchild",
            "JeffriArt",
            "Zainokta Rakei",
            "KucingLaut",
          ]
        },
        {
          'title': "Tester",
          'list' : [
            "Asukiri",
            "Chero",
            "LegoVW",
            "Pika",   
            "Azureos",
            "Arcana",
            "Chumacera",
            "Feeltis Catcus",
            "Kisaragi Ryuu",
            "Rad",
            "Rediffract",
            "Ryokhan",
          ]
        },
        {
          'title': "Background Music",
          'subtitle': "DOVA",
          'class': "credit-bgm-section",
          'list': [
            {
              "[Machina]": "だんご工房",
              "[珊瑚礁]": "伊藤ケイス",
              "[隠された遺跡の中へ]": "秦暁",
              "[Clouded Stage]": "MFP [Marron Fields Production]",
              "[淵に立つ背中を押すのは...的なBGM]": "鷹尾まさき[タカオマサキ]",
              "[哀愁漂うホラー的なBGM]": "鷹尾まさき[タカオマサキ]",
              "[Digital Space]": "Shimtone",
              "[Painted from Memory ～想いは調べにのって～]": "田中芳典"
            },
            {
              "[閉ざされた海底都市]": "ハヤシユウ",
              "[堕ちる]": "もっぴーさうんど",
              "[Ruins]": "Motomuge",
              "[Tension]": "こおろぎ",
              "[帰ろう]": "T12ya",
              "[日曜の午後]": "KK",
              "[Funny Life]": "T.tam",
              "[South Wind]": "FLASH☆BEAT"
            }
          ]
        },
        {
          'title': "Ending Music Theme",
          'subtitle': "「Bersama Ioforia」",
          'list': {
            'Lyrics': [
              "Airani Iofifteen Channel hololive-ID",
              "Rikka ch.律可"
            ],
            'Music': "Rikka ch.律可",
            'Arrangement': 'Zuvanix'
          }
        },
        {
          'title': "Special Thanks",
          'content': [
            "Zad",
            "Binyo Boss", 
            "GrilledCheez",
            "Melon Juice", 
            "Rarana", 
            "Zuvanix", 
            "Kyarudesu",
            "Mas Ittou",
            "<img src='../images/holoid.png'>",
            "And All Beloved Ioforia..."
          ]
        },
      ],
      'quote': {
        'content': [`"Kehidupan sama dengan lukisan cat minyak. Kesalahanya memang tidak bisa dihapus, tapi bisa diperbaiki."`],
        'source': "Agnes Jessica"
      }
    }
  },
  "stop music CreditsSong",

]);